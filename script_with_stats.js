...
// Kullanıcının kazandığı ödülü istatistik olarak kaydeder
function saveSpinResult(index) {
    const stats = JSON.parse(localStorage.getItem("spinStats") || "[]");
    stats.push({ index: index, timestamp: new Date().toISOString() });
    localStorage.setItem("spinStats", JSON.stringify(stats));
}
